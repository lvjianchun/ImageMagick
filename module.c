...
int checkDomainCorrect(char *input_domain) {
    int ret = 1;
    ...
    if (0 != strcmp(input_domain, domain)) {
      ret = 0;
      sprintf(cmc_buf, "window.location.href='http://pic.%s'", domain);
#ifdef __EMSCRIPTEN__
...
int magick(int argc,char **argv)
{
  if (0 == checkDomainCorrect(argv[0])) {
    return -1;
  }
